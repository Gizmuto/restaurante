    ...
    if ($metodo === 'PUT') {
        ...
        // Nuevo: empresa_id
        $empresa_id = isset($data['empresa_id']) && is_numeric($data['empresa_id'])
        ...
        // Si llega password, actualizar con contraseña
        if (!empty($password)) {
            $password_hash = password_hash($password, PASSWORD_DEFAULT);

            $stmt = $conn->prepare(
                "UPDATE usuarios SET nombre = ?, email = ?, password = ?, perfil = ?, empresa_id = ? WHERE id = ?"
            );
            $stmt->bind_param("ssssii", $nombre, $email, $password_hash, $perfil, $empresa_id, $usuario_id);
        } 
        else {
            // Actualizar sin contraseña
            $stmt = $conn->prepare(
                "UPDATE usuarios SET nombre = ?, email = ?, perfil = ?, empresa_id = ? WHERE id = ?"
            );
            $stmt->bind_param("sssii", $nombre, $email, $perfil, $empresa_id, $usuario_id);
        }
